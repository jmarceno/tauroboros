...
use rocket::{get, post, put, patch, Route};
...
async fn get_planning_prompt(state: &State<AppStateType>) -> ApiResult<Json<PlanningPrompt>> {
    ...
    prompt.map(|p| Json(p))
        .ok_or_else(|| ApiError::not_found("Planning prompt not found")
        .with_code(ErrorCode::PlanningPromptNotConfigured))
}

#[get("/api/planning/prompts")]
async fn get_all_planning_prompts(state: &State<AppStateType>) -> ApiResult<Json<Vec<PlanningPrompt>>> {
...
async fn update_planning_prompt(
    ...
    let prompt_changed = req.prompt_text.as_ref()
...
async fn get_prompt_versions(state: &State<AppStateType>, key: String) -> ApiResult<Json<Vec<PlanningPromptVersion>>> {
...
async fn list_planning_sessions(state: &State<AppStateType>) -> ApiResult<Json<Vec<Value>>> {
    ...
    let with_urls: Vec<Value> = sessions.iter().map(|s| {
...
async fn create_planning_session(
    ...
    // Check prompt exists
    let prompt: Option<PlanningPrompt> = sqlx::query_as(
        "SELECT * FROM planning_prompts WHERE key = ? AND is_active = 1 LIMIT 1"
    )
    .bind(prompt_key)
    .fetch_optional(&state.db)
    .await
    ...
    let session = PiWorkflowSession {
        ...
        cwd: req.cwd.clone().unwrap_or_else(|| state.project_root.clone()),
        worktree_dir: None,
        branch: None,
        pi_session_id: None,
...
async fn send_planning_message(
    ...
    if !matches!(session.session_kind, PiSessionKind::Planning | PiSessionKind::ContainerConfig) {
        return Err(ApiError::bad_request("Not a planning session")
            .with_code(ErrorCode::NotAPlanningSession));
    }
    
    // For now, just acknowledge - full planning session management would require
    // integration with the pi process
    let hub = state.sse_hub.read().await;
    let _ = hub.broadcast(&WSMessage {
...
async fn reconnect_planning_session(
    ...
    // Check if already active (placeholder logic)
    ...
    let updated = get_workflow_session(&state.db, &id).await?
...
async fn change_session_model(
    ...
    if let Some(ref level) = req.thinking_level {
        if !matches!(level, ThinkingLevel::Default | ThinkingLevel::Low | ThinkingLevel::Medium | ThinkingLevel::High) {
            return Err(ApiError::bad_request("Invalid thinkingLevel. Allowed values: default, low, medium, high")
...
async fn close_planning_session(state: &State<AppStateType>, id: String) -> ApiResult<Json<Value>> {
    ...
    .bind(PiSessionStatus::Completed)
    .bind(now)
    .bind(now)
    .bind(&id)
    .execute(&state.db)
...
async fn get_planning_timeline(state: &State<AppStateType>, id: String) -> ApiResult<Json<Vec<Value>>> {
    let session = get_workflow_session(&state.db, &id).await?
        .ok_or_else(|| ApiError::not_found("Session not found").with_code(ErrorCode::SessionNotFound))?;
...
async fn rename_planning_session(
    ...
    if req.name.trim().is_empty() {
        return Err(ApiError::bad_request("Name is required and must be a non-empty string"));
    }
