...
#[get("/api/sessions/<id>")]
async fn get_session(state: &State<AppStateType>, id: String) -> ApiResult<Json<PiWorkflowSession>> {
...
async fn get_session_timeline(state: &State<AppStateType>, id: String) -> ApiResult<Json<Vec<Value>>> {
    let session = get_workflow_session(&state.db, &id).await?
        .ok_or_else(|| ApiError::not_found("Session not found").with_code(ErrorCode::SessionNotFound))?;
    ...
    let timeline: Vec<Value> = messages.iter().map(|m| {
        json!({
            "id": m.id,
...
async fn get_session_usage(state: &State<AppStateType>, id: String) -> ApiResult<Json<Value>> {
    ...
    let total_prompt: i64 = messages.iter().filter_map(|m| m.prompt_tokens.map(|v| v as i64)).sum();
    let total_completion: i64 = messages.iter().filter_map(|m| m.completion_tokens.map(|v| v as i64)).sum();
    let total_cache_read: i64 = messages.iter().filter_map(|m| m.cache_read_tokens.map(|v| v as i64)).sum();
    let total_cache_write: i64 = messages.iter().filter_map(|m| m.cache_write_tokens.map(|v| v as i64)).sum();
    let total_tokens: i64 = messages.iter().filter_map(|m| m.total_tokens.map(|v| v as i64)).sum();
    ...
    match req.event_type.as_str() {
        ...
        "message" => {
            ...
            let message = SessionMessage {
                ...
                role: req.role.clone().map(|r| match r.as_str() {
                    "user" => MessageRole::User,
                    "assistant" => MessageRole::Assistant,
                    "system" => MessageRole::System,
                    _ => MessageRole::Assistant,
                }).unwrap_or(MessageRole::Assistant),
                event_name: req.event_name.clone().or_else(|| Some(req.event_type.clone())),
                message_type: req.message_type.clone().map(|t| match t.as_str() {
                ...
                content_json: req.content_json.clone()
                    .map(|c| c.to_string())
                    .unwrap_or_else(|| format!("{{\"text\":\"{}\"}}", req.text.clone().unwrap_or_default())),
                model_provider: req.model_provider.clone().or_else(|| usage.get("provider").and_then(|v| v.as_str().map(String::from))),
                model_id: req.model_id.clone().or_else(|| usage.get("model").and_then(|v| v.as_str().map(String::from))),
                agent_name: req.agent_name.clone(),
                prompt_tokens: usage.get("input").and_then(|v| v.as_i64().map(|n| n as i32)),
                completion_tokens: usage.get("output").and_then(|v| v.as_i64().map(|n| n as i32)),
                cache_read_tokens: usage.get("cacheRead").and_then(|v| v.as_i64().map(|n| n as i32)),
                cache_write_tokens: usage.get("cacheWrite").and_then(|v| v.as_i64().map(|n| n as i32)),
                total_tokens: usage.get("totalTokens").and_then(|v| v.as_i64().map(|n| n as i32)),
                cost_json: if cost.is_object() { Some(cost.to_string()) } else { None },
                ...
                session_status: None,
                workflow_phase: None,
                raw_event_json: Some(serde_json::to_value(req.into_inner()).unwrap_or_default().to_string()),
            };
        ...
        "complete" => {
            let final_status = req.status.clone().unwrap_or_else(|| "completed".to_string());
            ...
            hub.broadcast_status(&id, &final_status, Some(finished_at)).await;
            
            Ok(Json(json!({ "ok": true })))
        }
        
        _ => Err(ApiError::bad_request(format!("Unsupported event type: {}", req.event_type))
            .with_code(ErrorCode::InvalidRequestBody)),
    }
}

#[get("/api/task-runs/<id>/messages")]
async fn get_task_run_messages(state: &State<AppStateType>, id: String) -> ApiResult<Json<Vec<SessionMessage>>> {
