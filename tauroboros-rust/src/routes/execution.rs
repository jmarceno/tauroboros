...
async fn archive_run_direct(state: &State<AppStateType>, id: String) -> ApiResult<Json<Value>> {
    ...
    if matches!(run.status, WorkflowRunStatus::Queued | WorkflowRunStatus::Running | WorkflowRunStatus::Stopping | WorkflowRunStatus::Paused) {
        return Err(ApiError::conflict("Only completed or failed workflow runs can be archived")
