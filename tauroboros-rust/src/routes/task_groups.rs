...
async fn broadcast_members_added(state: &State<AppStateType>, group_id: &str, task_ids: &[String], count: i32) {
    let hub = state.sse_hub.read().await;
    let _ = hub.broadcast(&WSMessage {
...
async fn broadcast_members_removed(state: &State<AppStateType>, group_id: &str, task_ids: &[String], count: i32) {
    let hub = state.sse_hub.read().await;
    let _ = hub.broadcast(&WSMessage {
...
async fn create_group(state: &State<AppStateType>, req: Json<CreateGroupRequest>) -> ApiResult<Json<TaskGroup>> {
    // Validate name
    ...
    if let Some(ref color) = req.color {
        if !validate_hex_color(color) {
            return Err(ApiError::bad_request("color must be a valid hex color (e.g., #888888)")
    ...
    if let Some(status) = req.status {
        if !matches!(status, TaskGroupStatus::Active | TaskGroupStatus::Completed | TaskGroupStatus::Archived) {
            return Err(ApiError::bad_request("status must be active, completed, or archived")
    ...
    let member_task_ids = if let Some(ref task_ids) = req.task_ids {
        ...
        let invalid: Vec<&String> = task_ids.iter().filter(|id| !valid_ids.contains(*id)).collect();
        if !invalid.is_empty() {
            return Err(ApiError::bad_request(format!(
                "Invalid task IDs: {}", invalid.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", ")
    ...
    let group = create_task_group(&state.db, &req.name.trim(), req.color.clone(), member_task_ids).await?;
...
async fn update_group(
    ...
    let updated = update_task_group(&state.db, &id, req.name.clone().map(|s| s.trim().to_string()), req.color.clone(), req.status).await?
...
async fn add_tasks(
    ...
    if req.task_ids.is_empty() {
        return Err(ApiError::bad_request("taskIds array is required").with_code(ErrorCode::InvalidRequestBody));
    }
    ...
    let invalid: Vec<&String> = req.task_ids.iter().filter(|id| !valid_ids.contains(*id)).collect();
    if !invalid.is_empty() {
        return Err(ApiError::bad_request(format!(
            "Invalid task IDs: {}", invalid.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", ")
        )).with_code(ErrorCode::InvalidRequestBody));
    ...
    let updated = get_task_group(&state.db, &id).await?
