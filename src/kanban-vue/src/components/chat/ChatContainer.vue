...
<template>
  <div style="display: contents;">
    ...
    <button
      ...
      class="chat-toggle"
      :style="{ right: planningChat.isOpen.value ? panelWidth + 'px' : '0' }"
      @click="planningChat.openPanel()"
    >
      <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
      </svg>
      <span>Chat</span>
      <span v-if="planningChat.sessions.value.length > 0" class="chat-toggle-badge">
    ...
    <div
      v-if="planningChat.isOpen.value"
      :class="['chat-panel', { open: planningChat.isOpen.value, resizing: isResizing }]"
      :style="{ width: panelWidth + 'px', right: planningChat.isOpen.value ? '0' : '-' + panelWidth + 'px' }"
    >
      <!-- Resize Handle -->
      <div
      ...
      <div class="chat-tabs-container">
        ...
        <div class="chat-tabs-primary">
          <button
            ...
            :key="session.id"
            :class="['chat-session-tab', { active: planningChat.activeSessionId.value === session.id }]"
        ...
        <div class="chat-tabs-secondary">
          <div class="flex items-center gap-2">
            <button class="new-session-btn" @click="createNewChat">
            ...
            <button
              ...
              :class="{ 'text-accent-primary': activeTab === 'chat' }"
            ...
            <button
              class="new-session-btn"
              :class="{ 'text-accent-primary': activeTab === 'sessions' }"
              @click="activeTab = 'sessions'"
            >
              History
              <span v-if="activeSessionsCount > 0" class="px-1.5 py-0.5 text-xs bg-accent-success/20 text-accent-success rounded-full">
                {{ activeSessionsCount }}
              </span>
            </button>
          </div>
      ...
      <div v-if="activeTab === 'chat'" class="flex-1 overflow-hidden relative">
        ...
        <div
          v-else
          class="h-full flex flex-col items-center justify-center text-dark-text-muted p-4"
        >
      ...
      <div v-else-if="activeTab === 'sessions'" class="flex-1 overflow-y-auto bg-dark-bg">
        ...
        <!-- Empty State -->
        <div v-else-if="allSessions.length === 0" class="flex flex-col items-center justify-center py-12 text-dark-text-muted">
          <svg class="w-12 h-12 mb-3 opacity-30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                ...
                <span
                  ...
                  class="px-1.5 py-0.5 text-xs bg-accent-success/20 text-accent-success rounded"
                >
                  Active
                </span>
              </div>
              <div class="text-xs text-dark-text-secondary space-y-0.5">
                ...
                <p v-if="session.model && session.model !== 'default'" class="text-dark-text-muted">
                  Model: {{ session.model }}
                </p>
              </div>
            </div>
    ...
    <div
      v-if="planningChat.isOpen.value && planningChat.minimizedSessions.value.length > 0"
    ...
    >
      ...
      >
        ...
        <span v-if="session.messages.length > 0" class="min-badge">
    ...
    >
      <div class="bg-dark-surface border border-dark-border rounded-lg shadow-xl w-[400px] max-w-[90vw] p-4">
        ...
        <div class="flex items-center justify-end gap-2 mt-4">
          <button class="btn btn-sm" @click="cancelModelSelection">
            Cancel
          </button>
          <button
