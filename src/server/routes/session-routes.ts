...
export function registerSessionRoutes(router: Router, ctx: ServerRouteContext): void {
  ...
  router.get("/api/tasks/:id/messages", ({ params, json, db }) => json(db.getSessionMessageViewsByTask(params.id)))
  router.get("/api/task-runs/:id/messages", ({ params, json, db }) => json(db.getSessionMessageViewsByTaskRun(params.id)))
