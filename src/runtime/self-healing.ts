...
export class SelfHealingService {
  ...
  async investigateFailure(input: InvestigateFailureInput): Promise<SelfHealingInvestigationResult> {
    const options = this.db.getOptions()
    const source = await this.resolveSourceContext(input.run.id)
  ...
  private async resolveSourceContext(runId: string): Promise<SourceContext> {
    ...
    try {
      if (!existsSync(cloneDir)) {
        const cloneResult = await runCommand(["git", "clone", "--depth", "1", this.githubUrl, cloneDir], this.projectRoot)
      ...
      return {
        ...
        sourcePath: null,
        githubUrl: this.githubUrl,
        notes: `Unable to clone source locally: ${message}`,
      }
    }
  }
}
